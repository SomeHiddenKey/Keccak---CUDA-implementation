...
void hostr(const char* path, int times=1){
    for(int tm = 0; tm<times; tm++){
        ...
        char* contents = new char[rateBytes];
        ...
        char* input_d;
        cudaMalloc((void**)&input_d, (rateBytes) * sizeof(char));
        ...
        int offset = 0;
        while(size>0){
            double hashed = pbuf->sgetn(contents, rateBytes);
            cudaMemcpyAsync(input_d, contents, rateBytes, cudaMemcpyHostToDevice);  
...
int main( int argc, char *argv[] ){
    if(LOGG)myfile.open ("b_1s1t.csv");
