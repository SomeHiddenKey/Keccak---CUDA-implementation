...
void f_function(union INTER iState, union INTER* State){
    ...
    for(int round = 0; round<rounds; round++){
            ...
            for(int i = 0; i<25; i++){
                indx = pi[i];
                iState._64[i] = ROT(state._64[indx], rho[indx]); //not correct
            }
...
void Keccak(string_t input, int size, unsigned char* output){
...
int main(){
    ...
    Keccak(input, sizeof(message)-1, output);
    for(int i = 0; i<32; i++)
        printf("%02x",output[i]);
    printf("\n");
}