...
__global__ void Keccak(string_t input, int size, union INTER* State){
    __shared__ union INTER iState, jState; 
    memset(&iState, 0, 200);
    ...
    uint8_t i_5 = i*blockDim.x;
        ...
        for(int round = 0; round<rounds; round++){
            ...
            for(int x = 0; x<5; x++){
                indx = pi[i_5+x];
                jState._64[i_5+x] = ROT(iState._64[indx], rho[indx]);
            }
            //__syncthreads();
            ...
            for(int x = 0; x<5; x++)
            {
                iState._64[x+i_5] = jState._64[x+i_5] ^ (~jState._64[(x+1)%5 + i_5] & jState._64[(x+2)%5 +i_5]);
            }
            //__syncthreads();
...
void hostr(const char* path, int times=8){
    for(int tm = 0; tm<times; tm++){
        ...
        // //call device kernal
        Keccak<<<1,5>>>(input_d, sze, State_d);
