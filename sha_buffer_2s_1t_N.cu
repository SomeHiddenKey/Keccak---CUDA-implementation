...
void hostr(const char* path, int times=1){
    for(int tm = 0; tm<times; tm++){
        ...
        for(int offset = 0; size>0; itter = !itter){
            ...
            Keccak<<<1,1,0,stream_k>>>((itter?input_d2:input_d1), size+rateBytesBuffer, State_d);
        ...
        Keccak<<<1,1,0,stream_c>>>((itter?input_d2:input_d1), size+rateBytesBuffer, State_d); 
...
int main( int argc, char *argv[] ){
    if(LOGG)myfile.open ("b_2s1t.csv");
